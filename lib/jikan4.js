...
class Jikan4 {
    ...
    async getAnimeSearch({
        ...
        if (genresExclude != "") {
            url += `&genresExclude=${genresExclude}`;
        }
    ...
    async getMagazines({
        ...
        let url = `${this.baseURL}/magazines?page=${page}&limit=${limit}`;
        return this.get(url);
    }
    ...
    async get(url) {
        ...
        while(this.autoRateLimit && res.status === 429) {
        ...
        if (res.status !== 200) {
            ...
            // check for status error in API data(sometime API does not include proper status code)
            ...
            if (data.status == undefined || data.status == 200) {
                ...
                if (this.#cacheConfig.useCache) {
                    while (true) {
                        try {
                            if (
                                this.#getCacheKeysFromStorage().length >
                                this.#cacheConfig.limit
                            ) {
